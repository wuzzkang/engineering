...
- **V2 Mobile Split View & Interactive Preview Toggle**: Set default mobile split screen ratio on `/generate/v2` to 85% Form / 15% Preview Bar (Image 2) so all component section accordions, brief field, URL slug, and action buttons are fully visible on initial load. Added an interactive `👁️ Preview` toggle button on the `LIVE SANDBOX PREVIEW V2` header bar that smoothly switches split ratio to 15.5% Form / 84.5% Preview (Image 3), positioning the preview bar FLUSH directly underneath the global color selection buttons (`Navy`, `Emerald`, `Amber`, `Purple`, `Rose`, `Slate`) with zero leakage of the AI Brief card or AI Ready badge. Fixed `toggleMobilePreviewRatio` handler so clicking `Tutup Preview` when in Full Preview mode (`mobileViewMode === 'preview'`) seamlessly returns to Split Screen mode (85/15). Compacted top header paddings, removed the resizer handle bar (`Seret Atas / Bawah`), and added `pb-16 lg:pb-0` bottom padding so the preview block sits cleanly above the mobile bottom navbar and central `+` FAB.
