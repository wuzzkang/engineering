...
| Last Updated | 2026-08-02 (V2 4 Hero Options Complete, 100vh Full Height, hero-basic BG Image Upload, LP Async Bugfix) |
...
- **Hero Section Options Evolution (V2)**: Successfully built, registered, and synchronized **Option 1: Modern Split Hero (`hero_split` / `hero-split`)**, **Option 2: Centered Impact Hero (`hero_centered` / `hero-centered`)**, and **Option 3: Conversion & Video Hero (`hero_video` / `hero-video`)**. Fully integrated across all 4 monorepo packages (`wuzzkang-sections`, `wuzzkang-dashboard`, `wuzzkang-lp`, `@wuzzkang/renderer-core` preview app). Option 3 features YouTube & Vimeo watch-to-embed auto-conversion, dark overlay opacity control, fallback thumbnail upload via `V2ImagePickerWidget`, trust badge pill, centered headline, subheadline, and CTA button. All 3 options feature 0% forced text fallbacks and 100% E2E test pass (1.65ms benchmark).
...
- **V2ImagePickerWidget Universal Hardening**: Elevated `V2ImagePickerWidget` to the universal image upload standard for all V2 sections. Changes: (1) **Delete-from-Supabase Bug Fix** — tombol hapus sebelumnya hanya clear UI state; kini memanggil `DELETE /api/media` dengan `storagePath` sebelum clear state; (2) **Auto-Delete on Image Replace** — helper `_deleteOldStorageFile()` dipanggil otomatis saat user upload file baru, pilih Unsplash, atau paste URL baru — mencegah storage leak (foto lama menumpuk di Supabase); (3) **4-Layer File Security Validation** — Layer 1: size hard limit 3MB; Layer 2: MIME type allowlist; Layer 3: Magic bytes validation (reads 12 bytes header fisik, tidak bisa dipalsukan dengan rename file); Layer 4: Canvas image decode test (mencegah polyglot / file corrupt); (4) **Canvas Auto-Resize** — file > 500KB di-compress otomatis via Canvas API (quality iteration 0.85→0.3, fallback scale dimensi 70%) sebelum upload; (5) **FileReader Fallback** — jika upload Supabase gagal, preview lokal instan via FileReader; (6) **UI Fix: Unsplash Panel** — input keyword dan tombol kini disusun vertikal (full-width) agar tombol tidak terpotong di panel sempit; (7) **storagePath Prop** — prop baru `storagePath` + internal `_deleteOldStorageFile()` tracking untuk lifecycle delete yang benar; (8) **hero-split EditorForm Refactored** — ~150 baris custom image DIY code dihapus dan diganti 10 baris `<V2ImagePickerWidget>`.
- **aiQuota Utility Extraction (Redis Quota Centralization)**: Ekstrak Lua atomic quota check script yang sebelumnya terduplikasi di 2 lokasi di `image.route.js` menjadi satu utility terpusat `src/utils/aiQuota.js`. Exposes `checkAndIncrementDailyQuota(userId, dailyLimit)` sebagai single source of truth untuk daily AI quota enforcement. Dokumentasi ⚠️ WARNING ditambahkan ke `utils/redis.js` dan `queues/queue.js` menjelaskan mengapa 2 Redis connection terpisah diperlukan (BullMQ blocking commands isolation).
